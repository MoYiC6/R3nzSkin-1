...
inline static void footer() noexcept
{
	...
	ImGui::textUnformattedCentered("交流群：963804237");
...
void GUI::render() noexcept
{
	...
	static const auto vector_getter_skin = [](void* vec, const std::int32_t idx, const char** out_text) noexcept {
		...
		*out_text = idx == 0 ? "默认" : vector.at(idx - 1).skin_name.c_str();
		return true;
		};
		...
		if (ImGui::BeginTabBar("TabBar", ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip)) {
			if (player) {
				if (ImGui::BeginTabItem("本地玩家")) {
					...
					if (ImGui::Combo("当前眼位皮肤", &cheatManager.config->current_combo_ward_index, vector_getter_ward_skin, static_cast<void*>(&cheatManager.database->wards_skins), cheatManager.database->wards_skins.size() + 1))
			...
			if (ImGui::BeginTabItem("Extras")) {
				ImGui::hotkey("菜单按键", cheatManager.config->menuKey);
				...
				ImGui::hoverInfo("它可以让你在不打开菜单的情况下，通过键盘指定的按键更换皮肤。");

				if (cheatManager.config->quickSkinChange) {
					ImGui::Separator();
					ImGui::hotkey("上一个皮肤按键", cheatManager.config->previousSkinKey);
					ImGui::hotkey("下一个皮肤按键", cheatManager.config->nextSkinKey);
					ImGui::Separator();
				}
				...
				ImGui::SliderFloat("字体比例", &cheatManager.config->fontScale, 1.0f, 2.0f, "%.3f");
